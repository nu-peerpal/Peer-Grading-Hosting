...
function Dashboard(props) {
  ...
  useEffect(() => {
    if (Cookies.get("userData") && !savedStudentId) {
      // create new user if not viewing as student and cookie is set
  ...
  useEffect(() => {
    if (courseId && isInstructor && courseName) {
      ...
      (async () => {
        try {
          const courseData = (
            await axios.get(`/api/courses?canvasId=${courseId}`)
          ).data.data;
          ...
          if (!courseData.length) {
            ...
            try {
              ...
              console.log({ err });
              console.log(
                `failed to create course ${courseId} [${courseName}]`
              );
            }
          }

          const [theCanvasUsers, dbUsers, dbEnrollments] = (
            await Promise.all([
...
=======
          const dbUserLookup = _.keyBy(dbUsers, ({ canvasId }) => canvasId);
          const newUsers = theCanvasUsers.filter(
            ({ canvasId }) => !dbUserLookup[canvasId]
          );
>>>>>>> 02a34fae201da07e42438f34efb1e7dea9c3c62e
          ...
          const dbEnrollmentLookup = _.keyBy(
            dbEnrollments,
            ({ userId, enrollment }) => [userId, enrollment]
          );
          const newEnrollments = theCanvasUsers.filter(
            ({ canvasId, enrollment }) =>
              !dbEnrollmentLookup[[canvasId, enrollment]]
          );

          if (newEnrollments.length) {
            const newEnrollmentsPayload = newEnrollments.map(
              ({ canvasId, enrollment }) => ({
                userId: canvasId,
                courseId,
                enrollment
            ...
            const newUsersPayload = newEnrollments.map(u => ({
              ...u,
            ...
            console.log(
              `adding ${newEnrollmentsPayload.length} new enrollments`
            );

            // add new users
            const newUsersResult = (
              await axios.post(
                "/api/users?type=multiple&filter=true",
                newUsersPayload
              )
            ).data;

            // add new enrollments
            const newEnrollmentsResult = (
              await axios.post(
                "/api/courseEnrollments?type=multiple&filter=true",
                newEnrollmentsPayload
              )
            ).data;
          ...
          console.log(
            "errors getting canvasUsers, dbUsers, or dbCourseEnrollments"
          );
          console.log(err);
  ...
  useEffect(() => {
    (async () => {
      ...
      const assignments = (
        await axios.get(`/api/assignments?courseId=${courseId}`)
      ).data.data;
      ...
      if (!props.ISstudent && !canvasAssignments) {
        // get canvas courses
        axios
          .get(`/api/canvas/assignments?type=multiple&courseId=${courseId}`)
          .then(response => {
            ...
            const reviewAssignmentIds = assignments.map(({ reviewCanvasId }) =>
              parseInt(reviewCanvasId)
            );
            ...
            const finishedCanvasAssignments = allCanvasAssignments.filter(
              ({ canvasId }) =>
                finishedAssignmentIds.includes(parseInt(canvasId))
            );

            // should be in a list of unconfigured assignments
            const unconfiguredCanvasAssignments = allCanvasAssignments.filter(
              ({ canvasId }) =>
                !allKnownAssignmentIds.includes(parseInt(canvasId))
            );
      ...
      for (const {
        ...
        if (reviewStatus < 9) {
          ...
          taToDoReviews.push({
            ...
            reviewDueDate: reviewDueDate,
            rubricId: rubricId,
            actionItem: taActionItem,
            reviewStatus,
            link: "/assignments/fullassignmentview/fullassignmentview"
          });

          tempStudentInProgressReviews.push({
            canvasId: id,
            name,
            assignmentDueDate: assignmentDueDate,
            reviewDueDate: reviewDueDate,
      ...
      const studentToDoReviews = tempStudentInProgressReviews.filter(
  ...
  if (props.ISstudent) {
    return (
      <div className="Content">
        ...
        <StudentViewOutline
          isStudent={props.ISstudent}
          SetIsStudent={props.SetIsStudent}
        />
      </div>
    );
  } else {
    // TA or Instructor View
    return (
      <div className="Content">
        {canvasAvailable || (
          <CanvasAPICheck
            canvasAvailable={canvasAvailable}
            statusMessage={canvasStatusMessage}
          />
        )}
        ...
        <CanvasAssignments
          ...
          assignments={canvasFinishedAssignments}
        />
        <CanvasAssignments
          name="Enablable Assignments"
...
function CanvasAPICheck(props) {
  if (props.canvasAvailable) return null;
    ...
    return (
      <ListContainer
        ...
        data={[
          {
            name:
              "Enable your first assignment for Peer Reviews under Canvas Assignments!"
          }
        ]}
        info="Get Started"
        link="/"
